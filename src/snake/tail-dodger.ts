import { IGameState,  EMoveTypes, IMoveInfo, EMoveDirections, IPoint } from "./Interfaces";
...
const TailDodger = class {
    ...
    constructor(xy: IPoint) {
        this.snakeHead = xy;
        this.steps = [];
        const height = StateAnalyzer.getHeight();
        const width = StateAnalyzer.getWidth();
    ...
    getShortestPath ( endXY: IPoint ) {
        //Create path planner
        var planner = createPlanner(this.knownCollisions);

        //Find path
        var path = [];
        let dist = planner.search((this.snakeHead.x), (this.snakeHead.y),  (endXY.x), (endXY.y),  path);
